...
SimpleDeltaSequence::ValuePair::ValuePair(int value, int bound) : value_(value), bound_(bound) {}
SimpleDeltaSequence::ValuePair::~ValuePair() {}

const char SimpleDeltaSequence::default_sep_char = ',';

SimpleDeltaSequence::SimpleDeltaSequence(const char sep_char) : sep_char_(sep_char), current_pos_(0) {}
SimpleDeltaSequence::~SimpleDeltaSequence() {}


SimpleDeltaSequence* SimpleDeltaSequence::CreateInstance(const char sep_char) {
...
bool SimpleDeltaSequence::empty_line(const std::string &line) {
    if (line.empty()) return true;
    size_t found = line.find_first_not_of("\t\n");
    return (found == string::npos);
}

void SimpleDeltaSequence::init_sequence() {
    // 获取输入文件的路径
    ...
    while (!seqf.eof()) {
        ...
        // v中的大小应该为2，即两个int值，一个为value，一个为bound
        assert(v.size() == 2);
...
void SimpleDeltaSequence::add_number(int v, int bound, int k) {
    SimpleDeltaSequence::ValuePair *p = new SimpleDeltaSequence::ValuePair(v, bound);
	seq_map_[k] = p;
}

int SimpleDeltaSequence::get_number_by_pos(int /*pos*/) {
    assert(0);
    return 0;
}

int SimpleDeltaSequence::get_number(int bound) {
    SimpleDeltaSequence::ValuePair *p = sequence_[current_pos_];
    assert("SimpleDeltaSequence: get_number p is NULL!" && p);
    int b = p->get_bound();
    // 判断bound是否和current_pos指向的值对的bound相同
...
void SimpleDeltaSequence::clear() {
...
void SimpleDeltaSequence::output_one(ostream &out, const SimpleDeltaSequence::ValuePair *p) {
    assert(p);
...
void SimpleDeltaSequence::get_sequence(ostream &out) {
...
unsigned INT64 SimpleDeltaSequence::sequence_length() {
